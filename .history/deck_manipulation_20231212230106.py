...
for nid, flds in query:
    ...
    english_match = re.match(r"[^▼]*?([a-zA-Z\s']+)[^▼]*?▼", string)

    # Check if English words are matched
    if english_match:
        english_words = english_match.group(1).strip()
        print(f"English Words: {english_words}")

        # Extract Chinese and Pinyin (excluding English words)
        chinese_pinyin = string.replace(english_match.group(1), '').strip()
        print(f"Chinese and Pinyin: {chinese_pinyin}")

        # Update the notes with the rearranged content
        cursor.execute("UPDATE notes SET flds=? WHERE id=? COLLATE unicase", (f"{chinese_pinyin}▼{english_words}", nid))
    else:
        print(f"No English words found in: {string}")
