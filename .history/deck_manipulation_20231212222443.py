...
for nid, flds in query:
    ...
    # Print the string and the match result for debugging
    ...
    if match:
        ...
        chinese_pinyin = flds.replace(match.group(0), '').strip()
        print(f"Back (Chinese and Pinyin): {chinese_pinyin}")

        cursor.execute("UPDATE notes SET flds=? WHERE id=? COLLATE unicase", (f"{chinese_pinyin}▼{front}", nid))
