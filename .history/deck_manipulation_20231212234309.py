...
for nid, flds in query:
    ...
    # Use Unicode representation for ▼ (U+25BC)
    unicode_separator = '\u25BC'

    # Iterate over characters to find the start of Chinese characters
    chinese_start = None
    for i, char in enumerate(string):
        if char == unicode_separator:
            chinese_start = i + 1
            break

    # Check if Chinese characters were found
    if chinese_start is not None:
        english_words = string[:chinese_start].strip("'").strip()
        chinese_pinyin = string[chinese_start:].strip()
        print(f"English Words: {english_words}")
        print(f"Chinese and Pinyin: {chinese_pinyin}")

        # Update the notes with the rearranged content
        cursor.execute("UPDATE notes SET flds=? WHERE id=? COLLATE unicase", (f"{chinese_pinyin}{unicode_separator}{english_words}", nid))
    else:
        print(f"No ▼ separator found in: {string}")
